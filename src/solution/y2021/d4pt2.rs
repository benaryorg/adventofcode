...
///
/// ```
/// # use adventofcode::solution::{ y2021::d4pt2::Solution, Solution as S };
/// # env_logger::init();
...
///     22 11 13  6  5\n\
///      2  0 12  3  7\n";
/// assert_eq!(Solution::new(input.to_string()).solve().unwrap(), "1924");
/// ```
pub struct Solution
{
	input: String,
...
impl super::super::Solution for Solution
{
	fn solve(&self) -> Result<String>
	{
		...
		let mut bingos = vec.into_iter().map(Bingo::from).collect::<Vec<_>>();
		...
		for number in numbers
		{
			...
			if bingos.len() == 1
			{
				if let Some(result) = bingos.first().unwrap().check(&set)
				{
			...
			{
				bingos.retain(|bingo| bingo.check(&set).is_none());
			}
