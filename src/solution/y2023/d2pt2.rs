...
///
/// ```
/// # use adventofcode::solution::{ y2023::d2pt2::Solution, Solution as S };
/// # env_logger::init();
/// let input = "\
...
/// assert_eq!(Solution::new(input.to_string()).solve().unwrap(), "2286");
/// ```
pub struct Solution
{
	input: String,
...
impl super::super::Solution for Solution
{
	fn solve(&self) -> Result<String>
	{
		...
		let result: usize = games.into_values()
			.map(|sets|
			{
				sets.iter()
					.flatten()
					...
					{
						acc.entry(colour.clone()).and_modify(|num| *num = *new_num.max(num)).or_insert(*new_num);
						acc
					})
					.into_values()
					.product::<usize>()
